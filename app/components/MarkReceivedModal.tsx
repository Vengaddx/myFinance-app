...
type Props = {
  open: boolean;
  onClose: () => void;
  onReceive: (amount: number, date: string, remarks: string) => Promise<void>;
  friendName: string;
  outstanding: number;
  currency: string;
};

export default function MarkReceivedModal({ open, onClose, onReceive, friendName, outstanding, currency }: Props) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await onReceive(amt, date, remarks);
    } finally {
      setSaving(false);
    }
  };
  ...
  const onFocus = (e: React.FocusEvent<HTMLInputElement>) => {
    e.currentTarget.style.borderColor = "rgba(52,199,89,0.7)";
    e.currentTarget.style.boxShadow = `0 0 0 3px ${isDark ? "rgba(52,199,89,0.2)" : "rgba(52,199,89,0.12)"}`;
  ...
  return (
    <>
      <style>{`.mrf-input::placeholder { color: ${isDark ? "rgba(235,235,245,0.28)" : "#aeaeb2"}; }`}</style>
      <div
        ref={overlayRef}
      ...
      >
        <div
          ...
          style={{
            background: modalBg, backdropFilter: "blur(48px) saturate(200%)",
            WebkitBackdropFilter: "blur(48px) saturate(200%)",
            borderRadius: 20, border: modalBorder,
            boxShadow: isDark
              ? "0 40px 80px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.05)"
              : "0 32px 64px rgba(0,0,0,0.12), 0 8px 24px rgba(0,0,0,0.06)",
            width: "100%", maxWidth: 420, maxHeight: "calc(100dvh - 48px)",
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 pt-6 pb-4 shrink-0" style={{ borderBottom: `1px solid ${divider}` }}>
            <div>
              ...
              <p className="text-[13px] mt-0.5" style={{ color: labelColor }}>{friendName}</p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="overflow-y-auto flex-1 px-6 py-4 flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                value={amount} onChange={(e) => { setAmount(e.target.value); setErrors({}); }}
            ...
            <div className="flex flex-col gap-2">
              <label className="text-[12px] font-semibold uppercase tracking-widest" style={{ color: labelColor }}>Date Received</label>
              <input
                type="date" value={date} onChange={(e) => setDate(e.target.value)}
                className="mrf-input" style={inputStyle()}
            ...
            <div className="flex flex-col gap-2">
              ...
              <input
                type="text" placeholder="Optional note" value={remarks}
          ...
          {/* Footer */}
          <div className="flex items-center justify-between px-6 pb-6 pt-4 gap-3 shrink-0" style={{ borderTop: `1px solid ${divider}` }}>
            <button
              onClick={onClose}
